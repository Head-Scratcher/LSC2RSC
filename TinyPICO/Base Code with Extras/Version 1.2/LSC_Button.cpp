...
LSC_Button::LSC_Button(uint8_t active) {
  // Active = LOW or HIGH (see comment above)
  _active = active;
...
void LSC_Button::onButtonPressed(buttonPressedCallback callback){ _onButtonPressed = callback; }
...
uint8_t * LSC_Button::_update(uint16_t button_value) 
{
  ...
  for (uint8_t i = 0; i < BUTTON_COUNT; i++)
  {
    ...
    // Increment our event delta for this button
